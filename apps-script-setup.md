...
// 폴더 안의 모든 여신 파일을 모아서 하나로 합쳐 돌려줌 (누가 올렸든 전부 다시 보이게 됨)
function doGet(e) {
  try {
    const folder = DriveApp.getFolderById(FOLDER_ID);
    ...
    return ContentService.createTextOutput(JSON.stringify({ goddesses: goddesses, cards: cards }))
      .setMimeType(ContentService.MimeType.JSON);
  } catch (err) {
...
- 주의 1: 누군가 카드를 **삭제**해서 보내도, 예전에 합쳐진 파일에는 그 카드가 남아있을 수 있어요(자동 삭제 반영은 안 됨) — 삭제는 나중에 파일을 열어 수동으로 정리하는 게 안전해요
